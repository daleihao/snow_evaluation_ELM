...
for season_i = 1:2
   ...
   ELM_fsnos_tmp(isUS<1) = nan;
    ...
    max_values = [1600 400 400 1 1];
    %%spatial_pattern
    %% plot 1
       filters = isnan(ELM_snowdepths_tmp) | isnan(UA_snowdepths_tmp) | isnan(SNODAS_snowdepths_tmp) |...
       ELM_snowdepths_tmp<=0 | UA_snowdepths_tmp<=0 | SNODAS_snowdepths_tmp<=0 |...
          ELM_fsnos_tmp<=0 | MODSCAG_fsnos_tmp<=0 | SPIRES_fsnos_tmp<=0;

    ELM_snowdepths_tmp(filters) = nan;
    UA_snowdepths_tmp(filters) = nan;
    SNODAS_snowdepths_tmp(filters) = nan;
           Areas_tmp = Areas;

        alloutputs = nan(2,5);
    alloutputs(1,:) = calStat(ELM_snowdepths_tmp,UA_snowdepths_tmp,Areas_tmp);
    alloutputs(2,:) = calStat(ELM_snowdepths_tmp,SNODAS_snowdepths_tmp,Areas_tmp);

    
    ax1 = subplot('position', [0.055 + 0.195*(season_i-1) 0.65 0.19 0.28]);
    
        Areas_tmp = Areas;
     Areas_tmp(isnan(ELM_snowdepths_tmp)) = nan;
    ...
    if(season_i==1)
        ...
        m_text(-111,33.2,num2str(nansum(ELM_snowdepths_tmp(:).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
    else
    ...
    if(season_i==1)
        ...
        m_text(-111,33.2,num2str(nansum((ELM_snowdepths_tmp(:)- UA_snowdepths_tmp(:)).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
    ...
    if(season_i==1)
        ...
        m_text(-111,33.2,num2str(nansum((ELM_snowdepths_tmp(:)- SNODAS_snowdepths_tmp(:)).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
