...
for season_i = 1:2
    ...
    if season_i==1
        snow_data_all_winter_forest = [];
        
        for forest_band = 1:5
            filters = fc_bands>=0 & ELM_surfacealbedos_tmp>0 & MCD43_surfacealbedos_tmp>0 & ...
               ELM_fsnos_tmp>0;
            
            ELM_surfacealbedos_tmp2 = ELM_surfacealbedos_tmp(filters);
            MCD43_surfacealbedos_tmp2 = MCD43_surfacealbedos_tmp(filters);

            ELM_surfacealbedos_tmp2(fc_bands(filters)~=forest_band) = nan;
            MCD43_surfacealbedos_tmp2(fc_bands(filters)~=forest_band) = nan;
            
            tmp = [ELM_surfacealbedos_tmp2; MCD43_surfacealbedos_tmp2];
            group_all_winter_forest = [ones(size(tmp,1)/2, 1); 2*ones(size(tmp,1)/2, 1)];
            
            snow_data_all_winter_forest =[snow_data_all_winter_forest tmp];
        end
    else
        
        snow_data_all_spring_forest = [];
        for forest_band = 1:7
            filters = fc_bands>=0 & ELM_surfacealbedos_tmp>0 & MCD43_surfacealbedos_tmp>0 & ...
               ELM_fsnos_tmp>0;
            ELM_surfacealbedos_tmp2 = ELM_surfacealbedos_tmp(filters);
            ...
            group_all_spring_forest = [ones(size(tmp,1)/2, 1); 2*ones(size(tmp,1)/2, 1)];
            
            snow_data_all_spring_forest =[snow_data_all_spring_forest tmp];
        end
    end
end
%% figure plot
...
figure
set(gcf,'unit','normalized','position',[0.1,0.1,0.65,0.95]);
set(gca, 'Position', [0 0 1 1])

%% plot 1
subplot('position', [0.07 0.77 0.43 0.175]);
hold on
...
subplot('position', [0.55 0.77 0.43 0.175]);
hold on
...
subplot('position', [0.07 0.54 0.43 0.175]);
hold on
...
%% plot 4
subplot('position', [0.55 0.54 0.43 0.175]);
hold on
...
subplot('position', [0.07 0.3 0.43 0.175]);
hold on
...
h = daboxplot(snow_data_all_winter_forest(:,:),'groups',group_all_winter_forest,'outsymbol','k+',...
    'xtlabels', condition_names,'color',colors,...
    'whiskers',1,'scatter',0,'jitter',0,'scattersize',13,'mean',1, 'outliers',0);
...
%% plot 6
subplot('position', [0.55 0.3 0.43 0.175]);
hold on

h = daboxplot(snow_data_all_spring_forest(:,:),'groups',group_all_spring_forest,'outsymbol','k+',...
    'xtlabels', condition_names,'color',colors,...
   'add_dis',-0.02,...
...
subplot('position', [0.07 0.06 0.43 0.175]);
hold on
...
xlim([0.5 5.5])
ylim([-0.1 0.9])
text(0.6,0.9-1*0.06,'(g)','fontsize',12,'fontweight','bold')
xlabel('{\it f}_{sno}','fontsize',10)
ylabel('{\it \alpha}_{sur}','fontsize',12)

%% plot 8
subplot('position', [0.55 0.06 0.43 0.175]);
hold on
...
xlim([0.5 5.5])
ylim([-0.1 0.9])
text(0.6,0.9-1*0.06,'(h)','fontsize',12,'fontweight','bold')
