...
for season_i = 1:2
    ...
    if season_i==1
        ...
        for forest_band = 1:5
            filters = fc_bands>=0 & ELM_swes_tmp>0 & UA_swes_tmp>0 & SNODAS_swes_tmp>0 & ...
            ELM_fsnos_tmp>0;
            
            ELM_swes_tmp2 = ELM_swes_tmp(filters);
            UA_swes_tmp2 = UA_swes_tmp(filters);
            SNODAS_swes_tmp2 = SNODAS_swes_tmp(filters);
            ELM_swes_tmp2(fc_bands(filters)~=forest_band) = nan;
            UA_swes_tmp2(fc_bands(filters)~=forest_band) = nan;
            SNODAS_swes_tmp2(fc_bands(filters)~=forest_band) = nan;
...
xlim([0.5 5.5])
ylim([0 600])
text(0.6,600-600*0.06,'(e)','fontsize',12,'fontweight','bold')
...
xlim([0.5 5.5])
ylim([0 600])
text(0.6,600 -600*0.06,'(f)','fontsize',12,'fontweight','bold');
xlabel('Forest cover (%)','fontsize',12)
