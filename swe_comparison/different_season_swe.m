...
for season_i = 1:4
    ...
    for month_i = seasons_all
        ...
        hold on
        if(season_i==1)
            plot_global_map_other(lats, lons, ELM_swes_tmp, min_values(1), max_values(1),strcat(index, GridNames{season_i}),1, 0);
        else
            plot_global_map_other(lats, lons, ELM_swes_tmp, min_values(1), max_values(1),strcat( index, GridNames{season_i}),0, 0);
        end
        ...
        hold on
        if(season_i==1)
            plot_global_map_other(lats, lons, UA_swes_tmp, min_values(2), max_values(2),"",1, 0);
        else
            plot_global_map_other(lats, lons, UA_swes_tmp, min_values(2), max_values(2),"",0, 0);
        end
        ...
        hold on
        if(season_i==1)
            plot_global_map_other(lats, lons, SNODAS_swes_tmp, min_values(3), max_values(3),"",1, 1);
        else
            plot_global_map_other(lats, lons, SNODAS_swes_tmp, min_values(3), max_values(3),"",0, 1);
        end
...
print(gcf, '-dtiff', '-r300', 'different_swe.tif')
