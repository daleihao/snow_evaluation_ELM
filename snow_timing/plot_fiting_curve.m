...
for year_i = [2015:2015]
    ...
    % 32 85 95 45 94 35 90 70 150
    for row_i = 86:86
        for col_j = 149:149
            doys = 1:daynum;

            load(['../../../../MODIS/daily_data/MODSCAG_daily_' num2str(year_i-1) '.mat']);
            ...
            if(sum(filter)>30)
                ...
                if((gof.rsquare > 0.95) && (gof.rmse < 0.02) && ((estimates(1) - estimates(end))>0.05))
                    ...
                    plot([1 730],[1.0 1.0],'-k')

                    plot([snowmelt_doy+365 snowmelt_doy+365], [0 1.0],'-','linewidth',6,'color',[0 0 1 0.4]);
                    plot([mdl.d+365 mdl.d+365], [0 1.0],'-','linewidth',6,'color',[0 0 1 0.4]);
                    plot([MODSCAG_start_of_snow MODSCAG_start_of_snow], [0 1.0],'-','linewidth',6,'color',[0 0 1 0.4])
                    plot([MODSCAG_end_of_snow+365 MODSCAG_end_of_snow+365], [0 1.0],'-','linewidth',6,'color',[0 0 1 0.4])

                                        
                    text(snowmelt_doy+4+365, 0.94, 'Depletion onset date','fontsize',12,'fontweight','bold');
                    text(mdl.d+4+365, 0.5, 'Midpoint date','fontsize',12,'fontweight','bold');
                    text(MODSCAG_start_of_snow+4, 0.1, 'Accumulation onset date','fontsize',12,'fontweight','bold');
                    text(MODSCAG_end_of_snow+4+365, 0.1, 'End date','fontsize',12,'fontweight','bold');
                    text(383, 0.1, 'Duration','fontsize',12,'fontweight','bold');
                    text(265, 1.05, 'Snow accumulation season','fontsize',16,'fontweight','bold');
                    text(470, 1.05, 'Snowmelt season','fontsize',16,'fontweight','bold');
