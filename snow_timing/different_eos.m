...
ELM_end_of_snow(isUS<1) = nan;
MODSCAG_end_of_snow(isUS<1) = nan;
SPIRES_end_of_snow(isUS<1) = nan;
...
min_values = [1 1 1];
max_values = [200 200 200];

%% plot 1
ax1 = subplot('position', [0.02 + 0.3*(1-1) 0.07 0.29 0.8]);
colormap(ax1, colors);
hold on
plot_global_map_other(lats, lons, ELM_end_of_snow, min_values(1), max_values(1),"ELM",1, 1);
...
colormap(ax2, colors);
hold on
plot_global_map_other(lats, lons, MODSCAG_end_of_snow, min_values(2), max_values(2),"MODSCAG",0, 1);
set(gca,'fontsize',8,'fontname','time new roman')
...
colormap(ax3, colors);
hold on
plot_global_map_other(lats, lons, SPIRES_end_of_snow, min_values(3), max_values(3),"SPIReS",0, 1);
set(gca,'fontsize',8,'fontname','time new roman')
