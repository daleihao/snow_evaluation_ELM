...
for season_i = 1:2
    ...
    alloutputs = nan(2,5);
    alloutputs(1,:) = calStat(ELM_fsnos_tmp,MODSCAG_fsnos_tmp, Areas_tmp);
    alloutputs(2,:) = calStat(ELM_fsnos_tmp,SPIRES_fsnos_tmp, Areas_tmp);
    
    
    
    min_values = [0 -0.4 -0.4 0 0];
    max_values = [1 0.4 0.4 1 1];
    %%spatial_pattern
    %% plot 1
    
    Areas_tmp = Areas;
    Areas_tmp(isnan(ELM_fsnos_tmp)) = nan;
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_fsnos_tmp, min_values(1), max_values(1),"(a)",1, 0);
        m_text(-110,33.2,num2str(nansum(ELM_fsnos_tmp(:).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
    else
        plot_global_map_other(lats, lons, ELM_fsnos_tmp, min_values(1), max_values(1),"(b)",0, 0);
        m_text(-110,33.2,num2str(nansum(ELM_fsnos_tmp(:).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
    ...
    ELM_fsnos_tmp(filters) = nan;
    MODSCAG_fsnos_tmp(filters) = nan;
    SPIRES_fsnos_tmp(filters) = nan;
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_fsnos_tmp- MODSCAG_fsnos_tmp, min_values(2), max_values(2),"(c)",1, 0);
        m_text(-110,33.2,num2str(nansum((ELM_fsnos_tmp(:)- MODSCAG_fsnos_tmp(:)).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
        
    else
        plot_global_map_other(lats, lons, ELM_fsnos_tmp - MODSCAG_fsnos_tmp, min_values(2), max_values(2),"(d)",0, 0);
        m_text(-110,33.2,num2str(nansum((ELM_fsnos_tmp(:)- MODSCAG_fsnos_tmp(:)).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_fsnos_tmp - SPIRES_fsnos_tmp, min_values(3), max_values(3),"(g)",1, 1);
        m_text(-110,33.2,num2str(nansum((ELM_fsnos_tmp(:)- SPIRES_fsnos_tmp(:)).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
        
    else
        plot_global_map_other(lats, lons, ELM_fsnos_tmp - SPIRES_fsnos_tmp, min_values(3), max_values(3),"(h)",0, 1);
        m_text(-110,33.2,num2str(nansum((ELM_fsnos_tmp(:)- SPIRES_fsnos_tmp(:)).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
    ...
    m_text(-110,33.2,num2str(nansum((Rtmp(:).*Areas_tmp(:))),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
...
print(gcf, '-dtiff', '-r300', '../../figure_all_tif/fsno_spaital_pattern_temporal_correlation.tif')
