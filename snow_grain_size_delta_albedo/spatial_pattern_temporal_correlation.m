...
for season_i = 1:2
    ...
    %%spatial_pattern
    %% plot 1
       filters = isnan(ELM_grainsizes_tmp) | isnan(MODSCAG_grainsizes_tmp) | isnan(SPIRES_grainsizes_tmp) |...
       ELM_grainsizes_tmp<=0 | MODSCAG_grainsizes_tmp<=0 | SPIRES_grainsizes_tmp<=0 |...
        ELM_fsnos_tmp<=0 | MODSCAG_fsnos_tmp<=0 | SPIRES_fsnos_tmp<=0;

    ELM_grainsizes_tmp(filters) = nan;
    MODSCAG_grainsizes_tmp(filters) = nan;
    SPIRES_grainsizes_tmp(filters) = nan;
    
            Areas_tmp = Areas;

        alloutputs = nan(2,5);
    alloutputs(1,:) = calStat(ELM_grainsizes_tmp,MODSCAG_grainsizes_tmp,Areas_tmp);
    alloutputs(2,:) = calStat(ELM_grainsizes_tmp,SPIRES_grainsizes_tmp,Areas_tmp);

    ax1 = subplot('position', [0.055 + 0.195*(season_i-1) 0.65 0.19 0.28]);
    
        Areas_tmp = Areas;
     Areas_tmp(isnan(ELM_grainsizes_tmp)) = nan;
     Areas_tmp = Areas_tmp./nansum(Areas_tmp(:));

    colormap(ax1, colors_1);
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_grainsizes_tmp, min_values(1), max_values(1),"(a)",1, 0);
        m_text(-111,33.2,num2str(nansum(ELM_grainsizes_tmp(:).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
    else
        plot_global_map_other(lats, lons, ELM_grainsizes_tmp, min_values(1), max_values(1),"(b)",0, 0);
        m_text(-111,33.2,num2str(nansum(ELM_grainsizes_tmp(:).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_grainsizes_tmp- MODSCAG_grainsizes_tmp, min_values(2), max_values(2),"(c)",1, 0);
        m_text(-111,33.2,num2str(nansum((ELM_grainsizes_tmp(:)- MODSCAG_grainsizes_tmp(:)).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
        
    else
        plot_global_map_other(lats, lons, ELM_grainsizes_tmp - MODSCAG_grainsizes_tmp, min_values(2), max_values(2),"(d)",0, 0);
        m_text(-111,33.2,num2str(nansum((ELM_grainsizes_tmp(:)- MODSCAG_grainsizes_tmp(:)).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
    ...
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_grainsizes_tmp - SPIRES_grainsizes_tmp, min_values(3), max_values(3),"(g)",1, 1);
        m_text(-111,33.2,num2str(nansum((ELM_grainsizes_tmp(:)- SPIRES_grainsizes_tmp(:)).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
        
    else
        plot_global_map_other(lats, lons, ELM_grainsizes_tmp - SPIRES_grainsizes_tmp, min_values(3), max_values(3),"(h)",0, 1);
        m_text(-111,33.2,num2str(nansum((ELM_grainsizes_tmp(:)- SPIRES_grainsizes_tmp(:)).*Areas_tmp(:)),'%4.1f'),'fontsize',10, 'fontweight', 'bold');
    ...
    %% plot 3
    load('grainsize_temporal_R_v2.mat');
    ...
    if(season_i==1)
        ...
        label_y = ylabel({'R_{ELM & STC-MODSCAG}'});
...
print(gcf, '-dtiff', '-r300', '../../figure_all_tif/grainsize_spaital_pattern_temporal_correlation.tif')
