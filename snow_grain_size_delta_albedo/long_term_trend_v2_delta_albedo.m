...
for season_i = 1:2
    ...
    if season_i==1
        ...
        for elevation_band = 1:7
            filters = elevations_bands>=0 & ELM_deltaAlbedos_tmp>=0 & MODSCAG_deltaAlbedos_tmp>=0 & SPIRES_deltaAlbedos_tmp>=0;
            
            ELM_deltaAlbedos_tmp2 = ELM_deltaAlbedos_tmp(filters);
            MODSCAG_deltaAlbedos_tmp2 = MODSCAG_deltaAlbedos_tmp(filters);
            SPIRES_deltaAlbedos_tmp2 = SPIRES_deltaAlbedos_tmp(filters);
            ELM_deltaAlbedos_tmp2(elevations_bands(filters)~=elevation_band) = nan;
            MODSCAG_deltaAlbedos_tmp2(elevations_bands(filters)~=elevation_band) = nan;
            SPIRES_deltaAlbedos_tmp2(elevations_bands(filters)~=elevation_band) = nan;
            
            tmp = [ELM_deltaAlbedos_tmp2; MODSCAG_deltaAlbedos_tmp2; SPIRES_deltaAlbedos_tmp2];
...
for season_i = 1:2
    ...
    if season_i==1
        ...
        for forest_band = 1:5
            filters = fc_bands>=0 & ELM_deltaAlbedos_tmp>0 & MODSCAG_deltaAlbedos_tmp>0 & SPIRES_deltaAlbedos_tmp>0 & ...
            ELM_fsnos_tmp>0;
            
            ELM_deltaAlbedos_tmp2 = ELM_deltaAlbedos_tmp(filters);
            MODSCAG_deltaAlbedos_tmp2 = MODSCAG_deltaAlbedos_tmp(filters);
            SPIRES_deltaAlbedos_tmp2 = SPIRES_deltaAlbedos_tmp(filters);
            ELM_deltaAlbedos_tmp2(fc_bands(filters)~=forest_band) = nan;
            MODSCAG_deltaAlbedos_tmp2(fc_bands(filters)~=forest_band) = nan;
            SPIRES_deltaAlbedos_tmp2(fc_bands(filters)~=forest_band) = nan;
...
plot(2001:2019,ELM_deltaAlbedos_yearly(:,1),'color',colors(1,:),'linewidth',1.5)
%plot(2001:2019,MODSCAG_deltaAlbedos_yearly(:,1),'color',colors(2,:),'linewidth',1.5)
plot(2001:2019,SPIRES_deltaAlbedos_yearly(:,1),'color',colors(3,:),'linewidth',1.5)
...
xlim([2001 2019])
ylim([0.0 0.02])
text(2001.5,0.02-0.02*0.06,'(a)','fontsize',12,'fontweight','bold')
ylabel('{\it R}_{sno}','fontsize',12)
IAV1 = std(ELM_deltaAlbedos_yearly(:,1)-nanmean(ELM_deltaAlbedos_yearly(:,2)),1);
%IAV2 = std(MODSCAG_deltaAlbedos_yearly(:,1)-nanmean(MODSCAG_deltaAlbedos_yearly(:,2)),1);
IAV3 = std(SPIRES_deltaAlbedos_yearly(:,1)-nanmean(SPIRES_deltaAlbedos_yearly(:,2)),1);
text(2001.7,0.02-0.02*0.18,['IAV\_ELM: ' num2str(IAV1,'%4.4f')],'fontsize',11,'fontweight','bold','color',colors(1,:))
%text(2001.7,0.02-0.02*0.28,['IAV\_MODSCAG: ' num2str(IAV2,'%4.4f')],'fontsize',11,'fontweight','bold','color',colors(2,:))
text(2001.7,0.02-0.02*0.28,['IAV\_SPIReS: ' num2str(IAV3,'%4.4f')],'fontsize',11,'fontweight','bold','color',colors(3,:))

xlabel('Year')

sen_ps1 = nan(3,2);
[taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub([[2001:2019]' ELM_deltaAlbedos_yearly(:,1)], 0.05, 0);
sen_ps1(1,:) = [sen sig];
[taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub([[2001:2019]' MODSCAG_deltaAlbedos_yearly(:,1)], 0.05, 0);
sen_ps1(2,:) = [sen sig];
[taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub([[2001:2019]' SPIRES_deltaAlbedos_yearly(:,1)], 0.05, 0);
sen_ps1(3,:) = [sen sig];
...
plot(2001:2019,ELM_deltaAlbedos_yearly(:,2),'color',colors(1,:),'linewidth',1.5)
plot(2001:2019,MODSCAG_deltaAlbedos_yearly(:,2),'color',colors(2,:),'linewidth',1.5)
plot(2001:2019,SPIRES_deltaAlbedos_yearly(:,2),'color',colors(3,:),'linewidth',1.5)
title('Spring')
xlim([2001 2019])
ylim([0.0 0.02])
box on
set(gca,'linewidth',1,'fontsize',10,'yticklabels',[])
text(2001.5,0.02-0.02*0.06,'(b)','fontsize',12,'fontweight','bold')

IAV1 = std(ELM_deltaAlbedos_yearly(:,2)-nanmean(ELM_deltaAlbedos_yearly(:,2)),1);
IAV2 = std(MODSCAG_deltaAlbedos_yearly(:,2)-nanmean(MODSCAG_deltaAlbedos_yearly(:,2)),1);
IAV3 = std(SPIRES_deltaAlbedos_yearly(:,2)-nanmean(SPIRES_deltaAlbedos_yearly(:,2)),1);
text(2001.7,0.02-0.02*0.18,['IAV\_ELM: ' num2str(IAV1,'%4.4f')],'fontsize',11,'fontweight','bold','color',colors(1,:))
text(2001.7,0.02-0.02*0.28,['IAV\_STC-MODDRFS: ' num2str(IAV2,'%4.4f')],'fontsize',11,'fontweight','bold','color',colors(2,:))
text(2001.7,0.02-0.02*0.38,['IAV\_SPIReS: ' num2str(IAV3,'%4.4f')],'fontsize',11,'fontweight','bold','color',colors(3,:))
...
xlim([0.5 6.5])
ylim([0 0.06])
text(0.6,0.06-0.06*0.06,'(c)','fontsize',12,'fontweight','bold')
...
xlim([0.5 6.5])
ylim([0 0.06])
text(0.6,0.06-0.06*0.06,'(d)','fontsize',12,'fontweight','bold')
...
xlim([0.5 5.5])
ylim([0 0.06])
text(0.6,0.06-0.06*0.06,'(e)','fontsize',12,'fontweight','bold')
...
xlim([0.5 5.5])
ylim([0 0.06])
text(0.6,0.06-0.06*0.06,'(f)','fontsize',12,'fontweight','bold');
xlabel('Forest cover (%)','fontsize',12)

print(gcf, '-dtiff', '-r300', '../../figure_all_tif/deltaAlbedo_trend_elevation_time.tif')
