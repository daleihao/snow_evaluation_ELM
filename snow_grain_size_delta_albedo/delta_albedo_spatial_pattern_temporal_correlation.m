...
for season_i = 1:2
    ...
    min_values = [0 -0.05 -0.05 0 0];
    max_values = [0.05 0.05 0.05 1 1];
    %%spatial_pattern
    %% plot 1
    filters = isnan(ELM_deltaAlbedos_tmp) | isnan(MODSCAG_deltaAlbedos_tmp) | isnan(SPIRES_deltaAlbedos_tmp) |...
       ELM_deltaAlbedos_tmp<0 | MODSCAG_deltaAlbedos_tmp<0 | SPIRES_deltaAlbedos_tmp<0 |...
        ELM_fsnos_tmp<=0 | MODSCAG_fsnos_tmp<=0 | SPIRES_fsnos_tmp<=0;

    ELM_deltaAlbedos_tmp(filters) = nan;
    MODSCAG_deltaAlbedos_tmp(filters) = nan;
    SPIRES_deltaAlbedos_tmp(filters) = nan;
    
        Areas_tmp = Areas;

            alloutputs = nan(2,5);
    alloutputs(1,:) = calStat(ELM_deltaAlbedos_tmp,MODSCAG_deltaAlbedos_tmp,Areas_tmp);
    alloutputs(2,:) = calStat(ELM_deltaAlbedos_tmp,SPIRES_deltaAlbedos_tmp,Areas_tmp);

    
     Areas_tmp(isnan(ELM_deltaAlbedos_tmp)) = nan;
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_deltaAlbedos_tmp, min_values(1), max_values(1),"(a)",1, 0);
        m_text(-111,33.2,num2str(nansum(ELM_deltaAlbedos_tmp(:).*Areas_tmp(:)),'%4.3f'),'fontsize',10, 'fontweight', 'bold');
    else
        plot_global_map_other(lats, lons, ELM_deltaAlbedos_tmp, min_values(1), max_values(1),"(b)",0, 0);
        m_text(-111,33.2,num2str(nansum(ELM_deltaAlbedos_tmp(:).*Areas_tmp(:)),'%4.3f'),'fontsize',10, 'fontweight', 'bold');
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_deltaAlbedos_tmp- MODSCAG_deltaAlbedos_tmp, min_values(2), max_values(2),"(c)",1, 0);
        m_text(-111,33.2,num2str(nansum((ELM_deltaAlbedos_tmp(:)- MODSCAG_deltaAlbedos_tmp(:)).*Areas_tmp(:)),'%4.3f'),'fontsize',10, 'fontweight', 'bold');
        
    else
        plot_global_map_other(lats, lons, ELM_deltaAlbedos_tmp - MODSCAG_deltaAlbedos_tmp, min_values(2), max_values(2),"(d)",0, 0);
        m_text(-111,33.2,num2str(nansum((ELM_deltaAlbedos_tmp(:)- MODSCAG_deltaAlbedos_tmp(:)).*Areas_tmp(:)),'%4.3f'),'fontsize',10, 'fontweight', 'bold');
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_deltaAlbedos_tmp - SPIRES_deltaAlbedos_tmp, min_values(3), max_values(3),"(g)",1, 1);
        m_text(-111,33.2,num2str(nansum((ELM_deltaAlbedos_tmp(:)- SPIRES_deltaAlbedos_tmp(:)).*Areas_tmp(:)),'%4.3f'),'fontsize',10, 'fontweight', 'bold');
        
    else
        plot_global_map_other(lats, lons, ELM_deltaAlbedos_tmp - SPIRES_deltaAlbedos_tmp, min_values(3), max_values(3),"(h)",0, 1);
        m_text(-111,33.2,num2str(nansum((ELM_deltaAlbedos_tmp(:)- SPIRES_deltaAlbedos_tmp(:)).*Areas_tmp(:)),'%4.3f'),'fontsize',10, 'fontweight', 'bold');
    ...
    %% plot 3
    load('deltaAlbedo_temporal_R_v2.mat');
...
print(gcf, '-dtiff', '-r300', '../../figure_all_tif/deltaAlbedo_spaital_pattern_temporal_correlation.tif')
