...
for season_i = 1:2
    ...
    max_values = [1.0 0.2 0.2 1 1];
    %%spatial_pattern
    %% plot 1
    filters = isnan(ELM_snowAlbedos_tmp) | isnan(MODSCAG_snowAlbedos_tmp) | isnan(SPIRES_snowAlbedos_tmp) |...
       ELM_snowAlbedos_tmp<0 | MODSCAG_snowAlbedos_tmp<0 | SPIRES_snowAlbedos_tmp<0 |...
        ELM_fsnos_tmp<=0 | MODSCAG_fsnos_tmp<=0 | SPIRES_fsnos_tmp<=0;

    ELM_snowAlbedos_tmp(filters) = nan;
    MODSCAG_snowAlbedos_tmp(filters) = nan;
    SPIRES_snowAlbedos_tmp(filters) = nan;
    
        Areas_tmp = Areas;

            alloutputs = nan(2,5);
    alloutputs(1,:) = calStat(ELM_snowAlbedos_tmp,MODSCAG_snowAlbedos_tmp,Areas_tmp);
    alloutputs(2,:) = calStat(ELM_snowAlbedos_tmp,SPIRES_snowAlbedos_tmp,Areas_tmp);

    
     Areas_tmp(isnan(ELM_snowAlbedos_tmp)) = nan;
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_snowAlbedos_tmp, min_values(1), max_values(1),"(a)",1, 0);
        m_text(-111,33.2,num2str(nansum(ELM_snowAlbedos_tmp(:).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
    else
        plot_global_map_other(lats, lons, ELM_snowAlbedos_tmp, min_values(1), max_values(1),"(b)",0, 0);
        m_text(-111,33.2,num2str(nansum(ELM_snowAlbedos_tmp(:).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_snowAlbedos_tmp- MODSCAG_snowAlbedos_tmp, min_values(2), max_values(2),"(c)",1, 0);
        m_text(-111,33.2,num2str(nansum((ELM_snowAlbedos_tmp(:)- MODSCAG_snowAlbedos_tmp(:)).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
        
    else
        plot_global_map_other(lats, lons, ELM_snowAlbedos_tmp - MODSCAG_snowAlbedos_tmp, min_values(2), max_values(2),"(d)",0, 0);
        m_text(-111,33.2,num2str(nansum((ELM_snowAlbedos_tmp(:)- MODSCAG_snowAlbedos_tmp(:)).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
    ...
    hold on
    if(season_i==1)
        plot_global_map_other(lats, lons, ELM_snowAlbedos_tmp - SPIRES_snowAlbedos_tmp, min_values(3), max_values(3),"(g)",1, 1);
        m_text(-111,33.2,num2str(nansum((ELM_snowAlbedos_tmp(:)- SPIRES_snowAlbedos_tmp(:)).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
        
    else
        plot_global_map_other(lats, lons, ELM_snowAlbedos_tmp - SPIRES_snowAlbedos_tmp, min_values(3), max_values(3),"(h)",0, 1);
        m_text(-111,33.2,num2str(nansum((ELM_snowAlbedos_tmp(:)- SPIRES_snowAlbedos_tmp(:)).*Areas_tmp(:)),'%4.2f'),'fontsize',10, 'fontweight', 'bold');
    ...
    %% plot 3
    load('snowalbedo_temporal_R_v2.mat');
    ...
    if(season_i==1)
        ...
        label_y = ylabel({'R_{ELM & STC-MODDRFS}'});
...
print(gcf, '-dtiff', '-r300', '../../figure_all_tif/snowAlbedo_spaital_pattern_temporal_correlation.tif')
